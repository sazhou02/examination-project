...
if(examinee_info.name!="无"){
  for(var i=0;i<examinee_info.assRes.length;i ++){
    // 判断当前体检人是否已经体检过该项目
    if (examinee_info.assRes[i].category_id === 9 && examinee_info.assRes[i].status === 0) {
      state.info.button9=false;state.info.text9="提交";
    }
    if (examinee_info.assRes[i].category_id === 11 && examinee_info.assRes[i].status === 0) {
      state.info.button11=false;state.info.text11="提交";
    }
    if (examinee_info.assRes[i].category_id === 12 && examinee_info.assRes[i].status === 0) {
      state.info.button12=false;state.info.text12="提交";
    }
    if (examinee_info.assRes[i].category_id === 13 && examinee_info.assRes[i].status === 0) {
      state.info.button13=false;state.info.text13="提交";
    }
    if (examinee_info.assRes[i].category_id === 14 && examinee_info.assRes[i].status === 0) {
      state.info.button14=false;state.info.text14="提交";
    }
  }
}
