...
let department_id=16;
const state = reactive({
  info: {
    modal:false,
    data7:[{item_id:0,value_type:0,value:Math.random(),text:"请输入"}],//&*&,按category编号
    button7:true,
    text7:"无该项目",
    div7:""
  }
...
if(examinee_info.name!="无"){
  for(var i=0;i<examinee_info.assRes.length;i ++){
    // 判断当前体检人是否已经体检过该项目
    if (examinee_info.assRes[i].category_id === 7 && examinee_info.assRes[i].status === 0) {
      state.info.button7=false;state.info.text7="提交";
    }
  }
}
...
//获取项目x检查指标、随机填数
let item_7 = (await $axios.post("item/list",{category_id:7})).data; //&*&
for (var i = 1; i < item_7.length; i++) { //&*&
    if(item_7[i].value_type==0){//&*&
      state.info.data7.push({item_id:i,value_type:0,value:Math.random(),text:""});//&*&
    }
    else{
      state.info.data7.push({item_id:i,value_type:1,value:0,text:"请输入"});//&*&
    }
}
...
async function submit7(){//&*&
  if(!state.info.button7){
    let category_id=7;//&*&
    let data=state.info.data7;//&*&
    let order_id=examinee_info.order_id;
    let start_id=item_7[0].id;
    ...
    state.info.button7=true;
    state.info.text7="已提交";
  }
  else{
...
<template>
  <div>
    ...
    <Grid :col="1">
      ...
      <!--第一框格:血常规-->
      <GridItem>
        <Form :style="{ 'visibility' : state.info.div7  }">
          <FormItem >
              <b style="text-align: left;display: inline-block;font-size: 20px;">项目一：眼科检查</b>
              <Button class="button" :disabled='state.info.button7' type="primary" @click="submit7()">{{state.info.text7}}</Button>
          </FormItem>
          <!--&*&-->
          <FormItem v-for="(i, index) in item_7" class="item"> 
            <Row class="text">
              <Col span="2">{{ index+1 }}</Col>
              <Col span="8">{{ i.name }}</Col>
              <!--&*&-->
              <Col v-if="i.value_type==0" span="10"><Input v-model="state.info.data7[index].value"></Input></Col>
              <!--&*&-->
              <Col v-else span="10"><Input v-model="state.info.data7[index].text"></Input></Col>
