...
let department_id=19,category_id=3;
const state = reactive({
  info: {
    modal:false,
    data3:[{item_id:0,value_type:0,value:Math.random(),text:"请输入"}],//&*&,按category编号
    button3:true,
    text3:"无该项目",
    div3:""
  }
...
if(examinee_info.name!="无"){
  for(var i=0;i<examinee_info.assRes.length;i ++){
    ...
    if (examinee_info.assRes[i].category_id === category_id && examinee_info.assRes[i].status === 0) {
      state.info.button3=false;state.info.text3="提交";
    }
  }
}
...
//获取项目x检查指标、随机填数
let item_3 = (await $axios.post("item/list",{category_id:category_id})).data; //&*&
for (var i = 1; i < item_3.length; i++) { //&*&
    if(item_3[i].value_type==0){//&*&
      state.info.data3.push({item_id:i,value_type:0,value:Math.random(),text:""});//&*&
    }
    else{
      state.info.data3.push({item_id:i,value_type:1,value:0,text:"请输入"});//&*&
    }
}
...
async function submit3(){//&*&
  if(!state.info.button3){
    let data=state.info.data3;//&*&
    let order_id=examinee_info.order_id;
    let start_id=item_3[0].id;
    ...
    state.info.button3=true;
    state.info.text3="已提交";
  }
  else{
...
<template>
  <div>
    ...
    <Grid :col="1">
      ...
      <!--第一框格:血常规-->
      <GridItem>
        <Form :style="{ 'visibility' : state.info.div3  }">
          <FormItem >
              <b style="text-align: left;display: inline-block;font-size: 20px;">项目一：外科检查</b>
              <Button class="button" :disabled='state.info.button3' type="primary" @click="submit3()">{{state.info.text3}}</Button>
          </FormItem>
          <!--&*&-->
          <FormItem v-for="(i, index) in item_3" class="item"> 
            <Row class="text">
              <Col span="2">{{ index+1 }}</Col>
              <Col span="8">{{ i.name }}</Col>
              <!--&*&-->
              <Col v-if="i.value_type==0" span="10"><Input v-model="state.info.data3[index].value"></Input></Col>
              <!--&*&-->
              <Col v-else span="10"><Input v-model="state.info.data3[index].text"></Input></Col>
