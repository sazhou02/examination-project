...
if(examinee_info.name!="无"){
  for(var i=0;i<examinee_info.assRes.length;i ++){
    // 判断当前体检人是否已经体检过该项目
    if (examinee_info.assRes[i].category_id === 4 && examinee_info.assRes[i].status === 0) {
      state.info.button4=false;state.info.text4="提交";
    }
  }
...
for (var i = 1; i < item_4.length; i++) {
    if(item_4[i].value_type==0){// 0:数值型  1:文字型
