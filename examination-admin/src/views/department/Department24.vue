...
if(examinee_info.name!="无"){
  for(var i=0;i<examinee_info.assRes.length;i ++){
    // 判断当前体检人是否已经体检过该项目
    if (examinee_info.assRes[i].category_id === 6 && examinee_info.assRes[i].status === 0) {
      state.info.button6=false;state.info.text6="提交";
    }
    if (examinee_info.assRes[i].category_id === 8 && examinee_info.assRes[i].status === 0) {
      state.info.button8=false;state.info.text8="提交";
    }
  }
}
//获取项目x检查指标、随机填数
